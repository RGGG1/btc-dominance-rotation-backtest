...
- Free/public plans only allow historical data for roughly the last 365 days
  *from today*, so we clamp the start date accordingly.
"""
...
# End date for the analysis (we'll clamp this to 'today' if it's in the future)
END_DATE           = "2025-11-01"
...
def get_effective_dates():
    """
    CoinGecko free/public plans typically limit historical range to roughly
    the last 365 days *from today*.

    We:
      - Clamp the end date to today if END_DATE is in the future.
      - Then clamp the start date to be no earlier than (end - 364 days).
      - Also respect your DESIRED_START_DATE if it's later.
    """
    ...
    # 365 days window -> end minus 364 (inclusive range)
    earliest_allowed = effective_end - timedelta(days=364)
